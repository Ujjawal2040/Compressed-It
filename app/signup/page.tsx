  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (data.session) {
        ...
        toast("Please check your email for a confirmation link.", { icon: "✉️" });
        router.push("/login");
      }
    }
    setLoading(false);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden bg-[#09090b]">
      ...
      <div className="glass-card w-full max-w-md p-8 relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-white/60 text-sm mt-2">Join CompressIt today</p>
        </div>
                ...
                {[1, 2, 3, 4].map((level) => (
                  <div 
                    ...
                    className={`h-full flex-1 rounded-full transition-colors ${
                      strength >= level * 25 
                        ? strength === 100 ? "bg-green-500" : strength >= 50 ? "bg-amber-500" : "bg-red-500"
                        : "bg-white/10"
                    }`}
                  />
                ))}
